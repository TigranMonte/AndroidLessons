...
public class DB {
    ...
    public static final String COMPANY_COLUMN_NAME = "_name";
    ...
    private static final String PHONE_TABLE_CREATE = "create table " + PHONE_TABLE +
            "(" + PHONE_COLUMN_ID + " integer primary key autoincrement, " + PHONE_COLUMN_NAME +
    ...
    public Cursor getCompanyData() {
        return mDB.query(COMPANY_TABLE, null, null, null,
                null, null, null);
    }
    // данные по телефонам конкретной группы
    public Cursor getPhoneData(long companyID) {
        return mDB.query(PHONE_TABLE, null, PHONE_COLUMN_COMPANY + " = " + companyID,
                null, null, null, null );
    }
    ...
    private class DBHelper extends SQLiteOpenHelper {
        ...
        public void onCreate(SQLiteDatabase db) {
            ...
            String[] phoneSams = new String[] { "Galaxy S II", "Galaxy Nexus", "Galaxy Note", "Wave" };
            ...
            cv.clear();
            for (int i = 0; i < phoneHTC.length; i++) {
                cv.put(PHONE_COLUMN_COMPANY, 1);
                cv.put(PHONE_COLUMN_NAME, phoneHTC[i]);
                db.insert(PHONE_TABLE, null, cv);
            }
            for (int i = 0; i < phoneSams.length; i++) {
                cv.put(PHONE_COLUMN_COMPANY, 2);
                cv.put(PHONE_COLUMN_NAME, phoneSams[i]);
                db.insert(PHONE_TABLE, null, cv);
            }
            for (int i = 0; i < phoneLG.length; i++) {
                cv.put(PHONE_COLUMN_COMPANY, 3);
                cv.put(PHONE_COLUMN_NAME, phoneLG[i]);
                db.insert(PHONE_TABLE, null, cv);
            }
        }
        @Override
